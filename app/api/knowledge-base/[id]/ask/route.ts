    ...
    try {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          confidence,
          responseTime,
          method: 'dataset_retrieval'
        }
      });

    } catch (difyError) {
      logger.warn('数据集检索失败，尝试回退方案', { error: difyError });
      ...
      try {
        ...
        if (difyResponse.ok) {
          ...
          const responseTime = (Date.now() - startTime) / 1000;
          const answer = responseData.answer || '抱歉，我无法回答您的问题。';
          ...
          try {
            ...
            logger.error('保存回退问答记录失败', { error: dbError });
          }
          ...
          return NextResponse.json({
            ...
            data: {
              ...
              confidence: 0.3,
              responseTime,
              method: 'app_api_fallback'
            }
          });
        } else {
          throw new Error('应用API也失败了');
        }
      } catch (fallbackError) {
        logger.error('所有API方案都失败', { error: fallbackError });

        // 最终回退到简单文本回复
        const responseTime = (Date.now() - startTime) / 1000;
        const fallbackAnswer = `我收到了您的问题："${question}"。\n\n目前知识库问答功能遇到技术问题，请稍后重试。您也可以：\n1. 前往项目文档页面查看相关分析结果\n2. 尝试重新表述您的问题\n3. 联系系统管理员\n\n感谢您的耐心等待！`;
        ...
        try {
          ...
          logger.error('保存失败回复记录失败', { error: dbError });
        }
