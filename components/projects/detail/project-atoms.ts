...
export const projectFilesAtomFamily = atomFamily((projectId: string) => atom<UIFile[]>([]), (a, b) => a === b);
                ...
                if (jsonMatch && jsonMatch[1]) {
                    ...
                    if (metadata.tiobItems && metadata.tiobItems.length > 0) {
                        ...
                        const itemsWithSource = metadata.tiobItems.map((item: any) => ({
                            ...item, sourceFile: file.originalName, meetingDate: meetingDate
                        }));
...
export const projectTiobItemsAtomFamily = atomFamily((projectId: string) => atom((get) => {
