import {projectTiobItemsAtomFamily, tiobItemsAtom} from "@/components/projects/detail/project-atoms";
import {Button} from "@/components/ui/button";
import {Table, TableBody, TableCell, TableHead, TableHeader, TableRow,} from "@/components/ui/table";
import {getItemCategoryName} from "@/lib/export-utils";
...
export function TIOBComp(props: {
    ...
    // 使用项目特定的原子化状态
    const [tiobItems] = useAtom(props.project?.name ? projectTiobItemsAtomFamily(props.project.name) : tiobItemsAtom // 如果没有项目ID，则使用全局原子状态作为备选
    );
    ...
    const exportTiobItems = (items: TIOBInterface[]) => {
        ...
        try {
            ...
            const exportData = items.map((item) => ({
                ...
                日期: item.meetingDate || "",
                类型: getItemCategoryName(item.categoryType),
                事项内容: item.details,
                金额: item.amount,
    ...
    return (<div className="flex flex-col space-y-4 w-full">
            ...
            <div className="overflow-x-auto">
                <Table className="w-full">
                    ...
                    <TableBody>
                        {tiobItems.map((item: any, index: any) => (<TableRow key={index}>
                                ...
                                <TableCell className="font-medium whitespace-nowrap">
                                    {getItemCategoryName(item.categoryType)}
                                </TableCell>
