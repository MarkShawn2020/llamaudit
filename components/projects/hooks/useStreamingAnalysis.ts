...
export function useStreamingAnalysis(
  ...
  }, [isAnalyzing, closeEventSource, fileTasks]);
    ...
    return (event: MessageEvent) => {
      ...
      setFileTasks(prevTasks => {
        ...
        if (message.answer) {
          ...
          // 同时更新全局结果（兼容旧代码）
          setStreamingResult(prevResult => prevResult + newChunk);
        }
      ...
      for (const fileId of fileIds) {
        ...
        try {
          ...
          setFileTasks(prevTasks => {
            ...
            const task = newTasks.get(fileId);
            if (task) {
              task.eventSource = fileEventSource;
              newTasks.set(fileId, task);
            }
            return newTasks;
          });
  ...
  }, [updateFilesStatus, closeEventSource]);
